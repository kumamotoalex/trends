...
def make_tweet_fn(text, time, lat, lon):
    ...
    "*** YOUR CODE HERE ***"
    def get_info(info):
        information={'text':text, 'time':time, 'lat': lat, 'lon':lon}
        return information[info]
    return get_info
    #bad to have many elif statements?
...
def make_sentiment(value):
    ...
    "*** YOUR CODE HERE ***"
    return value
# better to make an else statement or just a return at the bottom of funciton if "if" is not met?
def has_sentiment(s):
...
def analyze_tweet_sentiment(tweet):
    ...
    return make_sentiment(total/total_words)
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    def two_parts_of_list(lst,f):
        """Sums application of a function to a list involving two consecutive parts of a list"""
        result=0
        for i in range(len(lst)):
            result+=f(lst[i-1],lst[i])
        return result
    def cent_area(fst,scnd):
        return latitude(fst)*longitude(scnd) - latitude(scnd)*longitude(fst)    
    def cent_x(fst,scnd):
        return (latitude(fst)+latitude(scnd))*(latitude(fst)*longitude(scnd)-latitude(scnd)*longitude(fst))
    def cent_y(fst,scnd):
        return (longitude(fst)+longitude(scnd))*(latitude(fst)*longitude(scnd)-latitude(scnd)*longitude(fst))

    area=two_parts_of_list(polygon,cent_area)/2
    if area==0:
        return latitude(polygon[0]), longitude(polygon[0]), area

    cx= (two_parts_of_list(polygon,cent_x)/(6*area))
    cy= (two_parts_of_list(polygon,cent_y)/(6*area))
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    def parts_of_lists(lst1,lst2,f):
        """Sums resultant values of a function involving two lists"""
        result=0
        for i in range(len(lst1)):
            result+=f(lst1[i],lst2[i])
        return result

    cx,cy,area=[],[],[]
    for poly in polygons:
        cx1,cy1,area1=find_centroid(poly)
        cx= cx+ [cx1]
        cy= cy+ [cy1]
        area= area+ [area1]

    cxstate= parts_of_lists(cx,area, mul)/sum(area)
    cystate= parts_of_lists(cy,area, mul)/sum(area)

    return make_position(cxstate,cystate)
...
def group_by_key(pairs):
    ...
    dictionary = {key:[] for key, _ in pairs}
    for x,y in pairs:
        dictionary[x]=dictionary[x]+[y]
    return dictionary
...
def group_tweets_by_state(tweets):
    ...
    """
    "*** YOUR CODE HERE ***"
    state_centers= {state: find_state_center(us_states[state]) for state in us_states}
    state_lst=[]
    ...
    for tweet in tweets:
        ...
        for state in state_centers:
            new_state_distance=geo_distance(tweet_location(tweet),state_centers[state])
            
            if new_state_tweet==[] or \
                    min(new_state_distance, old_state_distance)==new_state_distance:
                new_state_tweet= [state, tweet]
                old_state_distance=geo_distance(state_centers[new_state_tweet[0]],tweet_location(tweet))
       
        state_lst+=[new_state_tweet]

    return group_by_key(state_lst)
